...
module "azurerm_virtual_network" {
  ...
  name                = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-vnw-${var.description}"), "rg-")
...
module "azurerm_public_ip" {
  ...
  name                = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-pip-${var.description}"), "rg-")
  location            = var.location
  resource_group_name = local.resource_group_name
  tags = local.tagset
}
module "azurerm_virtual_network_gateway" {
  ...
  name                 = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-vgw-${var.description}"), "rg-")
...
module "azurerm_storage_account" {
  ...
  location            = var.location
  name                = "st${local.name}diag-1"
  resource_group_name = local.resource_group_name
  tags = local.tagset
}
module "azurerm_log_analytics_workspace" {
  source              = "../azurerm_log_analytics_workspace"
  name                = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-log-${var.description}"), "rg-")
  location            = var.location
  resource_group_name = local.resource_group_name
  tags = local.tagset
}
module "azurerm_network_security_group" {
  ...
  name                = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-nsg-${var.description}"), "rg-")
  location            = var.location
...
module "azurerm_network_watcher" {
  ...
  name                = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-nw-${var.description}"), "rg-")
  location            = var.location
  resource_group_name = module.azurerm_resource_group.name
  tags = local.tagset
}
module "azurerm_network_watcher_flow_log" {
  ...
  description                             = var.description
  environment                             = var.environment
  name                                    = trimprefix(replace(local.resource_group_name, "${var.environment}-${var.description}", "${var.environment}-nwfl-${var.description}"), "rg-")
